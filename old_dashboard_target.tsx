...
export interface NotificationItem {
    id?: string;
    type: 'follow' | 'like' | 'comment' | 'mention' | 'booking' | string;
    fromUserName: string;
    message: string;
...
export default function Dashboard({
    ...
    const greetingEmoji = greetingKey === 'goodMorning' ? 'ΓÿÇ∩╕Å' :
                         greetingKey === 'goodAfternoon' ? '≡ƒîñ∩╕Å' :
                         greetingKey === 'goodEvening' ? '≡ƒîà' : '≡ƒîÖ';
    ...
    const GlassStatCard: React.FC<GlassStatCardProps> = ({ title, value, icon, gradient, onClick, trend, trendUp }) => (
        <motion.div
    ...
    const QuickActionButton: React.FC<QuickActionButtonProps> = ({ icon, label, description, onClick, color }) => (
        <motion.button
    ...
    const ActivityItem: React.FC<ActivityItemProps> = ({ notification, onClick, formatTime }) => {
                        ...
                        >
                            ...
                            <div className="divide-y dark:divide-gray-800">
                                {(!notifications || notifications.length === 0) ? (
                                    ...
                                    notifications.slice(0, 5).map((n: NotificationItem, i: number) => (
                                        <ActivityItem
                                            key={n.id || i}
                                            notification={n}
                                            onClick={() => setActiveTab('feed')}
                                ...
                                <div className="p-5">
                                    {studioRooms.length === 0 ? (
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            {studioRooms.map((room: any, idx: number) => (
                                                <motion.div
                                                    key={idx}
                        ...
                        >
                            ...
                            <div className="p-4 space-y-2">
                                {recentConvos.length === 0 ? (
                                    ...
                                    recentConvos.map((c: Conversation, i: number) => (
                                        <motion.div
                                            key={c.uid + i}
                    ...
                    <div className="space-y-6">
                        ...
                        >
                            ...
                            <div className="space-y-3">
                                ...
                                {userData?.accountTypes?.some((r: string) => ['Talent', 'Producer', 'Engineer', 'Composer'].includes(r)) && (
                                    <>
                                        <QuickActionButton
                                ...
                                {userData?.accountTypes?.some((r: string) => ['Label', 'Agent'].includes(r)) && (
                                    <QuickActionButton
                                ...
                                {!userData?.accountTypes?.some((r: string) => ['Talent', 'Producer', 'Engineer', 'Composer', 'Studio'].includes(r)) && (
                                    <QuickActionButton
                            ...
                            <div className="relative z-10 p-6">
                                ...
                                {trendingItem ? (
                                    <>
                                        ...
                                        <p className="text-white/60 text-sm mb-4">by {trendingItem.author} ΓÇó {trendingItem.price} Tokens</p>
                                    </>
                                ) : (
                                    <>
                        ...
                        >
                            ...
                            <div className="space-y-3">
                            {profileSchemas && (userData?.accountTypes || []).map((role: string) => {
                                    const data = subProfiles?.[role] || {};
                                    const schema = profileSchemas?.[role] || [];
                                    if (schema.length === 0) return null;

                                    const total = schema.filter((f: ProfileSchema) => !f.isToggle && f.type !== 'list').length || 1;
                                    const filled = schema.filter((f: ProfileSchema) => !f.isToggle && f.type !== 'list' && data[f.key] && data[f.key].length).length;
