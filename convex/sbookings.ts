...
export const getBookingsByClient = query({
  ...
  handler: async (ctx, args) => {
    const idToLookup = args.clientClerkId || args.clerkId || args.userId || args.clientId;
    if (!idToLookup) return [];

    let user = await ctx.db
      .query("users")
      .withIndex("by_clerk_id", (q) => q.eq("clerkId", idToLookup))
      .first();

    if (!user && ctx.db.normalizeId("users", idToLookup)) {
      user = await ctx.db.get(ctx.db.normalizeId("users", idToLookup)!);
    }
...
export const getUpcomingBookings = query({
  ...
  handler: async (ctx, args) => {
    const idToLookup = args.clerkId || args.userId || args.clientId;
    if (!idToLookup) {
      return [];
    }
    ...
    const filtered = bookings.filter((b) => b.date && b.date >= now);

    const enriched = await Promise.all(
      filtered.map(async (booking) => {
        const studio = await ctx.db.get(booking.studioId);
        const room = booking.roomId ? await ctx.db.get(booking.roomId) : null;
        const client = await ctx.db.get(booking.clientId);
        return {
          ...booking,
          startTime: booking.startTime || booking.time || "00:00",
          studioName: studio?.name || "Recording Studio",
          roomName: room?.name || "Studio Room",
          clientName: client?.displayName || client?.username || "Client",
...
/**
 * Quick walk-up / on-tablet room booking for Kiosk (EDU and Studio door displays)
 */
export const bookKioskRoom = mutation({
  args: {
    studioId: v.id("studios"),
    roomId: v.optional(v.id("rooms")),
  ...
  handler: async (ctx, args) => {
    ...
    const endTime = new Date(
      now.getTime() + args.durationMinutes * 60000
    ).toISOString();
    ...
    const bookingDocId = `kiosk_${Date.now()}_${Math.random()
      .toString(36)
      .substring(2, 7)}`;

    const bookingId = await ctx.db.insert("sbookings", {
      id: bookingDocId,
      studioId: args.studioId,
      roomId: args.roomId,
      clientId: studio.ownerId,
      serviceType: args.isClassBooking
        ? "Class / Lecture"
        : "Student Lab Session",
      date: today,
      time: timeStr,
      ...
      startTime,
      endTime,
      isClassBooking: args.isClassBooking ?? false,
      className: args.purpose,
      professorName: args.professorName,
      lessonPlan: args.lessonPlan,
      clientNotes: `Kiosk Walk-Up: ${args.studentName}${
