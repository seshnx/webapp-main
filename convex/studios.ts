...
export const getStudioPublicProfile = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (!studio) {
      try {
        const id = ctx.db.normalizeId("studios", args.slug);
        if (id) {
          studio = await ctx.db.get(id);
        }
      } catch {
    ...
    if (!studio || studio.deletedAt !== undefined) {
      return null;
    }
...
/**
 * Get aggregated Kiosk data for real-time tablet displays (lobby & door tablets)
 */
export const getKioskData = query({
  args: { studioSlugOrId: v.string() },
  handler: async (ctx, args) => {
    // 1. Resolve studio by slug or by ID
    let studio = await ctx.db
      .query("studios")
      .withIndex("by_slug", (q) => q.eq("slug", args.studioSlugOrId))
      .first();

    if (!studio) {
      try {
        const id = ctx.db.normalizeId("studios", args.studioSlugOrId);
        if (id) {
          studio = await ctx.db.get(id);
        }
      } catch {
    ...
    // 4. Fetch today's bookings & class schedule
    const today = new Date().toISOString().split("T")[0];
    const rawBookings = await ctx.db
      .query("sbookings")
      .withIndex("by_studio", (q) => q.eq("studioId", studio._id))
      .collect();

    const todayBookings = rawBookings
      .filter((b) => {
        if (
          b.status === "Cancelled" ||
          b.status === "cancelled" ||
        ...
        const bDate = b.date || (b.startTime ? b.startTime.split("T")[0] : null);
        return (
          bDate === today ||
          (b.startDate && b.endDate && today >= b.startDate && today <= b.endDate)
        );
      })
      .map((b) => {
        const room = rawRooms.find((r) => r._id === b.roomId);
        const startTime =
          b.startTime || (b.date && b.time ? `${b.date}T${b.time}:00` : undefined);
        const endTime =
          b.endTime ||
        ...
        return {
          id: String(b._id),
          roomId: b.roomId ? String(b.roomId) : "",
          roomName: room?.name || "Main Studio",
          startTime,
          endTime,
          status: normalizedStatus,
          serviceType: b.serviceType || "Session",
          isClassBooking: b.isClassBooking ?? false,
    ...
    // Map rooms for floorplan
    const rooms = rawRooms.map((r, index) => {
      const fpRoom = floorplan?.rooms?.find(
        (fpr) => fpr.id === r._id || fpr.name === r.name
      );
      return {
        id: String(r._id),
        name: r.name,
        ...
        hourlyRate: r.hourlyRate || studio?.hourlyRate || 0,
        layout: fpRoom?.layout || {
          x: 50 + (index % 3) * 200,
          y: 50 + Math.floor(index / 3) * 150,
          width: 180,
          height: 120,
          color: fpRoom?.color || "#3B82F6",
        },
      };
    });

    // Map floorplan walls & structures
    const walls = (floorplan?.walls || []).map((w, idx) => ({
      id: w.id || `wall_${idx}`,
      x1: w.startX ?? (w as any).x1 ?? 0,
      y1: w.startY ?? (w as any).y1 ?? 0,
      x2: w.endX ?? (w as any).x2 ?? 0,
      y2: w.endY ?? (w as any).y2 ?? 0,
      stroke: (w as any).stroke || "#4B5563",
      strokeWidth: w.thickness ?? (w as any).strokeWidth ?? 4,
    }));

    const structures = (floorplan?.structures || []).map((s, idx) => ({
      id: s.id || `struct_${idx}`,
      label: (s as any).label || s.type,
      layout: {
        x: s.x ?? (s as any).layout?.x ?? 0,
        y: s.y ?? (s as any).layout?.y ?? 0,
        width: s.width ?? (s as any).layout?.width ?? 30,
        height: s.height ?? (s as any).layout?.height ?? 30,
        color: (s as any).color || (s as any).layout?.color || "#9CA3AF",
      },
    }));
    ...
    return {
      studio: {
        ...
        contact: {
          email: studio.email,
          phone: studio.phoneCell || studio.phoneLand,
          website: studio.website,
        },
        kiosk: {
          eduMode: studio.kioskEduMode ?? false,
          networkName: studio.kioskNetworkName || null,
        },
      },
      rooms,
      floorplan: {
