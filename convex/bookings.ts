...
export const getTalentBookings = query({
  args: {
    talentClerkId: v.optional(v.string()),
  ...
  handler: async (ctx, args) => {
    const idToLookup = args.talentClerkId || args.clerkId || args.userId;
    ...
    if (!talent && ctx.db.normalizeId("users", idToLookup)) {
      talent = await ctx.db.get(ctx.db.normalizeId("users", idToLookup)!);
    }
...
export const getClientBookings = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (!client && ctx.db.normalizeId("users", idToLookup)) {
      client = await ctx.db.get(ctx.db.normalizeId("users", idToLookup)!);
    }
...
export const getUpcomingBookings = query({
  ...
  handler: async (ctx, args) => {
    ...
    const filtered = combined.filter((b) => b.date && b.date >= now);
    ...
    const enriched = await Promise.all(
      filtered.map(async (booking) => {
        ...
        return {
          ...
          talentPhoto: talent?.avatarUrl,
          talentClerkId: talent?.clerkId,
          studioName: booking.location || booking.serviceType || "Session",
        };
      })
    );
...
/**
 * Reject booking (alias for declineBooking)
 */
export const rejectBooking = declineBooking;
