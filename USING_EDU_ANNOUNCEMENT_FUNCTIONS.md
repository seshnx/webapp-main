...
function BroadcastFeed({ schoolId, userId, userType }: {
  ...
  const unreadBroadcasts = useQuery(api.eduAnnouncements.getUnreadBroadcasts, {
    userId,
    schoolId,
    userType,
  });

  const allBroadcasts = useQuery(api.eduAnnouncements.getActiveBroadcasts, {
    schoolId,
  });

  const readBroadcasts = useQuery(api.eduAnnouncements.getUserReadBroadcasts, {
    userId,
    schoolId,
  });

  const markAsRead = useMutation(api.eduAnnouncements.markBroadcastAsRead);

  const handleMarkAsRead = async (broadcastId: Id<"eduAnnouncements">) => {
...
function CreateBroadcastForm({
  ...
  const createBroadcast = useMutation(api.eduAnnouncements.createBroadcast);
  const publishBroadcast = useMutation(api.eduAnnouncements.publishBroadcast);
...
function BroadcastDashboard({ schoolId }: { schoolId: Id<"schools"> }) {
  ...
  const [selectedBroadcast, setSelectedBroadcast] = useState<Id<"eduAnnouncements"> | null>(null);

  // Fetch broadcasts by status
  const publishedBroadcasts = useQuery(api.eduAnnouncements.getBroadcastsBySchool, {
    schoolId,
    status: 'published',
  });

  const draftBroadcasts = useQuery(api.eduAnnouncements.getDraftBroadcasts, {
    schoolId,
  });

  const scheduledBroadcasts = useQuery(api.eduAnnouncements.getScheduledBroadcasts, {
    schoolId,
  });

  const archivedBroadcasts = useQuery(api.eduAnnouncements.getBroadcastsBySchool, {
    schoolId,
    status: 'archived',
  });

  const stats = useQuery(api.eduAnnouncements.getBroadcastStats, {
    schoolId,
  });

  const deleteBroadcast = useMutation(api.eduAnnouncements.deleteBroadcast);
  const archiveBroadcast = useMutation(api.eduAnnouncements.archiveBroadcast);
  const publishBroadcast = useMutation(api.eduAnnouncements.publishBroadcast);
...
function AdminBroadcastCard({
  ...
  const reads = useQuery(api.eduAnnouncements.getBroadcastReads, {
...
export const publishScheduledBroadcasts = action({
  ...
  handler: async (ctx) => {
    ...
    const scheduled = await ctx.runQuery(api.eduAnnouncements.getScheduledBroadcasts, {});

    for (const broadcast of scheduled) {
      if (broadcast.scheduledFor && broadcast.scheduledFor <= now) {
        await ctx.runMutation(api.eduAnnouncements.publishBroadcast, {
          broadcastId: broadcast._id,
