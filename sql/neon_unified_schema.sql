...
CREATE TABLE IF NOT EXISTS clerk_users (
    id UUID PRIMARY KEY, -- Clerk user ID (UUID format)
...
CREATE TABLE IF NOT EXISTS follows (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    follower_id UUID NOT NULL REFERENCES clerk_users(id) ON DELETE CASCADE,
    following_id UUID NOT NULL REFERENCES clerk_users(id) ON DELETE CASCADE,
    created_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS notifications (
    ...
    actor_id UUID REFERENCES clerk_users(id) ON DELETE SET NULL,
    actor_name TEXT,
    actor_photo TEXT,
