...
export default function TechBookingModal({ user, userData, target, onClose }: TechBookingModalProps) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            // Create booking using hybrid Neon + MongoDB service
            await createBooking(
                {
                    sender_id: userData?.id || user?.uid,
                    target_id: target.id,
                    service_type: form.serviceCategory,
                    offer_amount: form.budgetCap ? parseFloat(form.budgetCap) : undefined,
                    message: form.issueDescription,
                    status: 'Pending',
                },
                {
                    studioType: 'tech_services',
                    // Store tech service metadata in MongoDB
            ...
            Sentry.captureMessage('Tech booking submitted successfully', 'info', {
                tags: { component: 'TechBookingModal' },
                extra: { serviceCategory: form.serviceCategory, targetId: target.id }
            });

            onClose?.();
        } catch (error) {
            console.error('Failed to submit tech booking:', error);
