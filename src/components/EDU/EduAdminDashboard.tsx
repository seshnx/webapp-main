...
export default function EduAdminDashboard({ user: propUser, userData: propUserData, allowedTabs, staffTitle, currentView }: EduAdminDashboardProps) {
    ...
    const [activeSchoolId, setActiveSchoolId] = useState<string | null>(contextSchoolId || null);
    const [activeSchoolData, setActiveSchoolData] = useState<SchoolData | null>(null);
        ...
        if (isGlobalAdminUser && supabase) {
            const fetchSchools = async () => {
                try {
                    ...
                    const schoolsList: SchoolData[] = schools || [];
    ...
    useEffect(() => {
        ...
        const loadSchoolMeta = async () => {
            try {
                ...
                if (school) {
                    setActiveSchoolData(school as SchoolData);
                }
            } catch (e) {
        ...
        try {
            ...
            const { error: roleError } = await supabase
                ...
                .insert({
                    ...
                    name: 'Admin',
                    color: '#dc2626',
                    permissions: SCHOOL_PERMISSIONS.map((p: any) => p.id)
                });
            ...
            setAllSchools(prev => [...prev, newSchool as SchoolData]);
                    ...
                    <>
                        ...
                        {activeTab === 'settings' && (
                            <EduSettings
                                ...
                                logAction={logAction}
                                refreshMeta={(newData) => setActiveSchoolData((prev: any) => ({...prev, ...newData}))}
                            />
                        )}
                    </>
                )}
