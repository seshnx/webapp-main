...
export default function BusinessOverview({ user, userData, setActiveTab }: BusinessOverviewProps) {
    ...
    const isLabel = userData?.accountTypes?.some((t: string) => ['Label', 'Agent'].includes(t));
    const isArtist = userData?.accountTypes?.some((t: string) => ['Talent', 'Producer', 'Engineer'].includes(t));
    ...
    useEffect(() => {
        ...
        fetchMetrics();
    }, [user?.id, user?.uid, isLabel, isStudio, hasDistribution, isTechnician]);
    ...
    return (
        <div className="space-y-6 animate-in fade-in">
            ...
            {isTechnician && (
                <TechOverviewCard data={metrics.tech} onNavigate={setActiveTab} />
            )}
