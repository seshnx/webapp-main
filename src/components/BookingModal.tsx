...
export default function BookingModal({ user, userData, target, onClose }: BookingModalProps) {
    ...
    const [bookOnBehalf, setBookOnBehalf] = useState<string>('me'); // 'me' or artistId
    ...
    const getServiceTypes = (): string[] => {
        const types = [...(SERVICE_TYPES as ServiceTypes).general];
        ...
        if (targetRoles.includes('Talent')) {
            ...
            if (['Singer', 'Vocalist', 'Singer-Songwriter', 'Backup Singer', 'Rapper'].includes(targetSubRole)) {
                types.push(...(SERVICE_TYPES as ServiceTypes).talent);
            }
            // DJ sub-roles
            else if (['DJ', 'Beatmaker'].includes(targetSubRole)) {
                types.push(...((SERVICE_TYPES as ServiceTypes).dj || []));
            }
            // Instrumentalist sub-roles
                ...
                types.push(...((SERVICE_TYPES as ServiceTypes).instrumentalist || []));
            }
            // Default talent types
            else {
                types.push(...(SERVICE_TYPES as ServiceTypes).talent, ...((SERVICE_TYPES as ServiceTypes).instrumentalist || []));
            }
        }

        // Producer-specific types
        if (targetRoles.includes('Producer')) {
            types.push(...((SERVICE_TYPES as ServiceTypes).production || []));
        }

        // Engineer-specific types
        if (targetRoles.includes('Engineer')) {
            types.push(...((SERVICE_TYPES as ServiceTypes).engineering || []));
        }

        // Studio-specific types (handled in main app)
        if (targetRoles.includes('Studio')) {
            types.push(...((SERVICE_TYPES as ServiceTypes).studio || []));
        }

        // Composer-specific types
        if (targetRoles.includes('Composer')) {
            types.push(...((SERVICE_TYPES as ServiceTypes).composer || []));
        }

        // Tech/Technician-specific types
        if (targetRoles.includes('Technician')) {
            types.push(...((SERVICE_TYPES as ServiceTypes).tech || []));
        }
    ...
    const handleDurationChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const dur = val === '' ? 0 : parseInt(val, 10);
        setForm(prev => ({
            ...prev,
            duration: dur,
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    onChange={e => setForm({...form, offerAmount: Number(e.target.value)})}
                                />
                            </div>
                        </div>
                    </div>
