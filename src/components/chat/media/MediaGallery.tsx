...
import { X, Image as ImageIcon, Video, FileText, Music, ChevronLeft, ChevronRight, Download, LucideIcon } from 'lucide-react';
...
export default function MediaGallery({
    ...
    useEffect(() => {
        ...
        const mediaItems: MediaItem[] = (mediaMessages || [])
            .filter((msg: any) => msg.media && !msg.deleted && !msg.deletedForAll)
            .map((msg: any) => ({
                id: msg._id,
                type: msg.media?.type || 'file',
                url: msg.media?.url || '',
                name: msg.media?.name,
                messageId: msg._id,
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[80] p-4">
            ...
            >
                ...
                <div className="p-3 border-b dark:border-gray-700 flex gap-2 overflow-x-auto scrollbar-hide shrink-0">
                    {[
                        { key: 'all' as MediaFilter, label: 'All', icon: null },
                        { key: 'images' as MediaFilter, label: 'Images', icon: ImageIcon },
                        { key: 'videos' as MediaFilter, label: 'Videos', icon: Video },
                        { key: 'audio' as MediaFilter, label: 'Audio', icon: Music },
                        { key: 'files' as MediaFilter, label: 'Files', icon: FileText }
                    ].map(f => {
                        const Icon = f.icon;
                        return (
                            <button
 ...
 */
function MediaViewer({ media, onClose, onDownload }: MediaViewerProps) {
