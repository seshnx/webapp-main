...
export interface ConversationData {
    ...
    n?: string; // name
    photo?: string;
    uid?: string; // other user's UID for direct chats
    lm?: string; // last message
    ls?: string; // last message sender
    lmt?: string | number; // last message timestamp
    uc?: number; // unread count
    [key: string]: any;
}
...
export interface ConversationItemProps {
    ...
    activeChat?: ConversationData | null;
...
export default function ConversationItem({
    ...
    const otherUserId = conversation.type === 'direct'
        ? (conversation.uid || (conversation.id.includes('_')
            ? conversation.id.split('_').find((id: string) => id !== currentUserId)
            : null))
        : null;
    ...
    const lastSeenDate = typeof lastSeen === 'number' ? new Date(lastSeen) : lastSeen;
            ...
            <div className="relative shrink-0">
                ...
                {(conversation.uc || 0) > 0 && (
                    <div className="absolute -top-1 -right-1 h-5 w-5 bg-brand-blue text-white text-[10px] font-bold flex items-center justify-center rounded-full border-2 border-white dark:border-[#2c2e36]">
                        {(conversation.uc || 0) > 99 ? '99+' : conversation.uc}
                    </div>
                )}
            </div>
