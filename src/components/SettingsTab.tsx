...
interface MarketplaceNotificationSettings {
    newOffers?: boolean;
    sales?: boolean;
    messages?: boolean;
}
...
interface ContentFilteringSettings {
...
export default function SettingsTab({
    ...
    const handleToggle = (category: keyof ExtendedSettings | null, key: string, subKey: string | null = null): void => {
        setLocalSettings(prev => {
            const updated = { ...prev };
            if (subKey && category) {
                const categoryObj = (updated as any)[category];
                if (categoryObj && typeof categoryObj === 'object') {
                    (updated as any)[category] = {
                        ...categoryObj,
                        [key]: {
                            ...categoryObj[key],
                            [subKey]: !categoryObj[key]?.[subKey]
                        }
                    };
                }
    ...
    const handleValueChange = (
        category: keyof ExtendedSettings | null,
        key: string,
        value: any,
    ...
    const updateAccountTypes = async (): Promise<void> => {
        ...
        try {
            ...
            // Find removed roles
            const removedRoles = (userData?.accountTypes || []).filter((role: AccountType) => !roles.includes(role));
    ...
    const cancelRoleRemoval = (): void => {
        setShowRoleRemoveModal(false);
        setRoleToRemove(null);
    };

    // Security handlers
    const openSecurityModal = (action: 'email' | 'password'): void => {
        setSecurityAction(action);
        setSecurityForm({ newEmail: '', newPassword: '', currentPassword: '' });
        setShowSecurityModal(true);
    };

    const handleSecurityUpdate = async (): Promise<void> => {
