// src/components/marketplace/ShippingVerification.tsx
...
/**
 * Shipping photo types
 */
export type ShippingPhotoType = 'packaging' | 'dropoff' | 'pickup' | 'unboxing';

/**
 * Shipping photo data structure
 */
export interface ShippingPhoto {
    url: string;
    timestamp: number;
    type: ShippingPhotoType;
}

/**
 * Shipping transaction data structure
 */
export interface ShippingTransaction {
    ...
    carrier?: string;
    packagingPhotos?: ShippingPhoto[];
    dropoffPhotos?: ShippingPhoto[];
    pickupPhotos?: ShippingPhoto[];
    unboxingPhotos?: ShippingPhoto[];
...
/**
 * Props for ShippingVerification component
 */
export interface ShippingVerificationProps {
    /** Transaction ID for shipping verification */
...
/**
 * Props for PhotoSection helper component
 */
...
/**
 * Photo capture mode configuration
 */
interface PhotoModeConfig {
    title: string;
    subtitle: string;
    tips: string[];
    minPhotos: number;
...
export default function ShippingVerification({
    ...
    onComplete,
    onMessage
}: ShippingVerificationProps) {
    const [transaction, setTransaction] = useState<ShippingTransaction | null>(null);
    const [loading, setLoading] = useState<boolean>(false);
    const [actionLoading, setActionLoading] = useState<boolean>(false);
    const [photoMode, setPhotoMode] = useState<ShippingPhotoType | null>(null);
    const [photos, setPhotos] = useState<ShippingPhoto[]>([]);
    ...
    const capturePhoto = useCallback(async () => {
        if (!videoRef.current || !canvasRef.current || !photoMode) return;
    ...
    const updateStatus = async (newStatus: string, additionalData: Record<string, any> = {}) => {
    ...
    const submitUnboxingPhotos = async (verified: boolean) => {
        if (photos.length < 2) {
    ...
    // Photo capture mode
    if (photoMode) {
        const modeConfig: Record<ShippingPhotoType, PhotoModeConfig> = {
            packaging: {
                            ...
                            <div className="p-6 space-y-6">
                                ...
                                <div className="relative bg-black rounded-xl overflow-hidden aspect-video">
                                    {showCamera ? (
                                        ...
                                        <img src={photos[photos.length - 1]?.url} className="w-full h-full object-contain" alt="Latest photo" />
                                    ) : (
                    ...
                    {transaction?.trackingNumber && (
                        <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-xl">
                            <div className="flex items-center justify-between">
                                ...
                                <button
                                    onClick={() => transaction?.trackingNumber && navigator.clipboard.writeText(transaction.trackingNumber)}
...
function PhotoSection({ title, photos, notes }: PhotoSectionProps) {
            ...
            {expanded && (
                <div className="p-3 space-y-3">
                    <div className="grid grid-cols-3 gap-2">
                        {photos.map((photo, idx) => (
                            <img key={idx} src={photo.url} className="w-full aspect-square object-cover rounded-lg" alt={`Photo ${idx + 1}`} />
                        ))}
                    </div>
                    {notes && (
