...
type TabId = 'store' | 'plans' | 'earnings';

// =====================================================
// COMPONENT
// =====================================================

export default function PaymentsManager({ user, userData }: PaymentsManagerProps): JSX.Element {
  ...
  const buyTokens = (pack: TokenPackage): void => {
    if (!pack.stripePriceId) {
      return alert("Configuration Error: This package is missing a Stripe Price ID.");
    }
    handleCheckout(pack.stripePriceId, 'payment');
  };

  const subscribeToPlan = (plan: TierPlan): void => {
    if (!plan.stripePriceId) {
      return alert("Configuration Error: This plan is missing a Stripe Price ID.");
    }
    handleCheckout(plan.stripePriceId, 'subscription');
  };

  const initCashOut = async (): Promise<void> => {
    if (!isTalent) return;
    if (walletData.payoutBalance <= 0) {
      return alert("No funds available for cash out.");
    }
  ...
  const sortedPlans = Object.values(tierData || {})
    .sort((a: TierPlan, b: TierPlan) => {
      if (a.id === currentTierId) return -1;
      if (b.id === currentTierId) return 1;
      return a.price - b.price;
    });

  const formatFee = (multiplier: number): string => {
      ...
      <div className="space-y-8">
        <div className="flex gap-4 border-b dark:border-gray-700 mb-8 overflow-x-auto">
          <button
            ...
            className={`pb-3 px-4 text-sm font-bold border-b-2 transition whitespace-nowrap ${
              activeTab === 'store'
          ...
          <div className="space-y-8 animate-in fade-in duration-300">
            ...
            {loadingConfig ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {(tokenPackages || []).map((pack: TokenPackage) => (
                  <div
                    key={pack.id}
                    className={`relative bg-white dark:bg-[#2c2e36] p-6 rounded-2xl border transition-all duration-200 hover:-translate-y-1 hover:shadow-xl ${
                      pack.highlight ? 'border-yellow-500 ring-2 ring-yellow-500/20' : 'dark:border-gray-700'
                    }`}
                  >
                    {pack.highlight && (
          ...
          <div className="space-y-8 animate-in fade-in duration-300">
            ...
            {loadingConfig ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                {sortedPlans.map((plan: TierPlan) => {
                  const isCurrent = plan.id === currentTierId;
                  return (
                    <div
                      key={plan.id}
                      className={`bg-white dark:bg-[#2c2e36] p-6 rounded-2xl border flex flex-col ${
                        isCurrent ? 'border-brand-blue ring-1 ring-brand-blue' : 'dark:border-gray-700'
                      } shadow-sm`}
                    >
                      {isCurrent && (
                        <div className="text-xs font-bold text-brand-blue uppercase tracking-wide mb-2 flex items-center gap-1">
                          <Star size={12} fill="currentColor"/> Current Plan
                        </div>
                      )}
                      ...
                      <button
                        ...
                        className={`w-full py-2.5 rounded-lg font-bold text-sm transition-colors mt-auto ${
                          isCurrent
          ...
          <div className="space-y-6 animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white dark:bg-[#2c2e36] p-6 rounded-2xl border dark:border-gray-700 shadow-sm relative overflow-hidden">
                ...
                <div className="text-xs bg-orange-50 dark:bg-orange-900/10 text-orange-700 dark:text-orange-300 p-3 rounded-lg flex items-start gap-2 mt-4">
                  <AlertCircle size={14} className="mt-0.5 shrink-0"/> Funds are released 24 hours after session completion.
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-[#2c2e36] p-6 rounded-2xl border border-green-500/30 shadow-lg relative overflow-hidden">
                ...
                <button
                  onClick={initCashOut}
                  disabled={processing || walletData.payoutBalance <= 0}
                  className="w-full bg-green-600 hover:bg-green-700 text-white py-3 rounded-xl font-bold shadow-lg hover:shadow-green-500/30 transition-all flex justify-center items-center gap-2 disabled:opacity-50 disabled:shadow-none"
                >
