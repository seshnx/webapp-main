...
import { Plus, Trash2, Settings, Zap, Music, PenTool, Clock, ChevronDown, Loader2, MapPin, Link as LinkIcon, DollarSign, Calendar, LucideIcon } from 'lucide-react';
...
interface NeedItem {
    ...
    type: string;
    value: string;
    gearSelection?: Record<string, any>;
}

/**
 * Props for BroadcastRequest component
 */
export interface BroadcastRequestProps {
    user?: any;
    userData?: any;
    onBack?: () => void;
    onSuccess?: () => void;
...
export default function BroadcastRequest({ user, userData, onBack, onSuccess }: BroadcastRequestProps) {

    // --- MAP & LOCATION STATE ---
    const [broadcastRangeIndex, setBroadcastRangeIndex] = useState<number>(2);
    const rangeValues = [5, 10, 15, 25, 50, 100, 250, 500];
    const currentRange = rangeValues[broadcastRangeIndex];

    const [location, setLocation] = useState<Location>({ lat: 34.0522, lng: -118.2437, name: 'Default' });
    ...
    const [action, setAction] = useState<string>('Play');
    ...
    const [paymentType, setPaymentType] = useState<string>('Flat Rate');
    ...
    const availableRoles = ACCOUNT_TYPES.filter((t: string) => t !== 'Technician');

    const getActionsForRole = (r: string): string[] => {
        switch (r) {
            case 'Talent': return [
                // Vocal-focused
    ...
    const updateNeed = (id: number, field: string, newValue: any) => setNeeds(needs.map(n => n.id === id ? { ...n, [field]: newValue } : n));

    const updateGearSelection = (id: number, level: string, value: any) => {
                            ...
                            {needs.map((need) => (
                                <div key={need.id} className="flex flex-col sm:flex-row gap-2 bg-gray-50 dark:bg-black/20 p-3 rounded-xl border dark:border-gray-700 animate-in slide-in-from-bottom-2">
                                    <div className="w-full sm:w-1/3 min-w-[140px]">
                                        <select className="w-full p-2 text-sm border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-[#2c2e36] dark:text-white focus:ring-2 focus:ring-brand-blue outline-none" value={need.type} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => updateNeed(need.id, 'type', e.target.value)}>
                                    ...
                                    <div className="flex-1 flex gap-2">
                                        {need.type === 'Gear' ? (
                                            ...
                                            <input className="w-full p-2 text-sm border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-[#2c2e36] dark:text-white placeholder-gray-400 focus:ring-2 focus:ring-brand-blue" placeholder="Describe requirement..." value={need.value} onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateNeed(need.id, 'value', e.target.value)} />
                                        )}
