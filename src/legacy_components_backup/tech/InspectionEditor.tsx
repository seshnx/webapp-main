...
/**
 * Props for InspectionEditor component
 */
export interface InspectionEditorProps {
    initialData?: InspectionData | null;
    onSave?: (data: InspectionData) => void;
    onCancel?: () => void;
    type: 'Pre' | 'Post';
}

export default function InspectionEditor({ initialData, onSave, onCancel, type }: InspectionEditorProps) {
    const [gearType, setGearType] = useState<string>(initialData?.gearType || 'guitar_electric');
    const [view, setView] = useState<string>('front');
    const [markers, setMarkers] = useState<DiagramMarker[]>(initialData?.markers || []);
    ...
    const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (res?.url) setPhotos(prev => [...prev, { url: res.url }]);
    };
