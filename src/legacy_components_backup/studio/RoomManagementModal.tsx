...
/**
 * Props for RoomManagementModal component
 */
export interface RoomManagementModalProps {
    room: RoomData;
    equipment: RoomEquipment[];
    setRoom: (room: RoomData) => void;
    setEquipment: (equipment: RoomEquipment[]) => void;
    onClose: () => void;
    onSave: () => void;
    onDelete: () => void;
    isNew: boolean;
...
/**
 * RoomManagementModal - Modal for managing studio rooms and their equipment
 */
export default function RoomManagementModal({ room, equipment, setRoom, setEquipment, onClose, onSave, onDelete, isNew }: RoomManagementModalProps) {

    const [selectedItem, setSelectedItem] = useState<SelectedGearItem | null>(null);
    const [quantity, setQuantity] = useState<number>(1);

    // Handler for the new Autocomplete component
    const handleGearSelect = (item: SelectedGearItem) => {
    ...
    const handleUpdateQuantity = (index: number, newQuantity: number) => {
        const updated = equipment.map((item, i) =>
            i === index ? { ...item, quantity: Math.max(1, parseInt(String(newQuantity)) || 1) } : item
        );
    ...
    const groupedInventory: Record<string, RoomEquipment[]> = equipment.reduce((acc, item) => {
        const cat = item.category || 'Other';
        if (!acc[cat]) acc[cat] = [];
        acc[cat].push(item);
        return acc;
    }, {} as Record<string, RoomEquipment[]>);
