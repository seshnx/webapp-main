// src/components/marketplace/PhotoVerification.tsx
 ...
 */
export type PhotoVerificationMode = 'capture' | 'compare' | 'review';

/**
 * Photo verification role
 */
export type PhotoVerificationRole = 'seller' | 'buyer';

/**
 * Verification result type
 */
export type VerificationResult = null | 'match' | 'mismatch' | 'unsure';
...
export interface VerificationPhoto {
    url: string;
    timestamp: number;
    type: 'departure' | 'inspection';
}

/**
 * Verification submission data
 */
export interface VerificationData {
    result: 'match' | 'mismatch' | 'unsure';
    notes: string;
    buyerPhotos: VerificationPhoto[];
    timestamp: number;
}

/**
 * Props for PhotoVerification component
 */
export interface PhotoVerificationProps {
    /** Current mode: capture (take photos), compare (side-by-side), or review (view results) */
    mode?: PhotoVerificationMode;
    /** Role: seller takes pre-departure photos, buyer takes inspection photos */
    role?: PhotoVerificationRole;
    /** Original listing photos for comparison */
    listingPhotos?: string[];
    /** Seller's pre-departure photos (for buyer comparison) */
    sellerDeparturePhotos?: VerificationPhoto[];
    /** Description of the item being verified */
    itemDescription?: string;
    /** Callback when photos are submitted */
    onPhotosSubmitted?: (photos: VerificationPhoto[], notes: string) => Promise<void> | void;
    /** Callback when verification is complete (buyer only) */
    onVerificationComplete?: (data: VerificationData) => Promise<void> | void;
    /** Minimum number of photos required */
    requiredPhotoCount?: number;
    /** Current user ID for upload path */
    userId?: string;
}
...
export default function PhotoVerification({
    ...
    const handleVerificationSubmit = async (result: 'match' | 'mismatch' | 'unsure') => {
