 ...
 */
export type ItemType = 'Presets' | 'Samples' | 'Beats';
 ...
 * Filter type
 */
export type FilterType = 'All' | 'Presets' | 'Samples' | 'Beats';

/**
 * Props for SeshFxStore component
 */
export interface SeshFxStoreProps {
...
/**
 * Props for SellItemModal component
 */
export interface SellItemModalProps {
    /** Current authenticated user */
    user: {
        id?: string;
        uid?: string;
...
export interface ItemDetailModalProps {
    ...
    /** Whether user owns this item */
    isOwned: boolean;
    /** Callback when modal is closed */
    onClose: () => void;
    /** Callback to purchase item */
    onBuy: () => void;
    /** Callback to download item */
    onDownload: () => void;
    /** Callback to report item */
    onReport: () => void;
}

/**
 * SeshFxStore Component
 *
 * Marketplace for buying/selling audio presets, samples, and beats.
 *
 * @example
 * <SeshFxStore
 *   user={currentUser}
 ...
 * />
 */
export default function SeshFxStore({ user, userData, tokenBalance }: SeshFxStoreProps) {
    ...
    const [reportTarget, setReportTarget] = useState<{ id: string; type: 'market_item'; summary: string } | null>(null);

    // Transform raw items and map ownership
    const items: MarketplaceItem[] = rawItems || [];
    const ownedItemIds = new Set((library || []).map((lib: LibraryItem) => lib.item_id));

    const handlePurchase = async (item: MarketplaceItem): Promise<void> => {
    ...
    const handleDownload = async (item: MarketplaceItem): Promise<void> => {
        try {
    ...
    const filteredItems = items.filter((i: MarketplaceItem) =>
        ...
        <div className="pb-32">
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-6">
                <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
                    {(['All', 'Presets', 'Samples', 'Beats'] as FilterType[]).map((f: FilterType) => (
...
function SellItemModal({ user, userData, onClose }: SellItemModalProps) {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                        <div className="border-2 border-dashed dark:border-gray-600 rounded-xl p-6 text-center hover:bg-gray-50 dark:hover:bg-gray-800 transition relative bg-blue-50/50 dark:bg-blue-900/10 h-full flex flex-col justify-center items-center">
                            <input type="file" className="absolute inset-0 opacity-0 cursor-pointer" accept="audio/*" onChange={e => setAudioFile(e.target.files?.[0] || null)} />
...
function ItemDetailModal({ item, isOwned, onClose, onBuy, onDownload, onReport }: ItemDetailModalProps) {
    return (
