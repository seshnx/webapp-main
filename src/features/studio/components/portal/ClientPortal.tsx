...
import { Calendar, DollarSign, User, Clock, CheckCircle, XCircle, AlertCircle, RefreshCw, Download, Plus, LucideIcon } from 'lucide-react';
...
type TabId = 'bookings' | 'rebook' | 'invoices' | 'profile';
...
interface RebookTabProps {
  templates: Template[];
  onRebook: (template: Template) => void;
 ...
 */
export default function ClientPortal({ user, userData }: ClientPortalProps): JSX.Element {
  ...
  const fetchPayments = async (): Promise<void> => {
    try {
      ...
      if (data.success) {
        ...
        const clientPayments = (data.data || []).filter((p: Payment) => clientBookingIds.includes(p.booking_id));
  ...
  const handleRebook = async (template: Template): Promise<void> => {
  ...
  const handleDownloadInvoice = (booking: Booking): void => {
 ...
 */
function BookingsTab({ bookings, formatDate, getStatusBadge, onDownloadInvoice }: BookingsTabProps): JSX.Element {
 ...
 */
function RebookTab({ templates, onRebook, formatCurrency }: RebookTabProps): JSX.Element {
  return (
    <div className="space-y-6">
      <div>
 ...
 */
function InvoicesTab({ payments, bookings, formatCurrency, formatDate }: InvoicesTabProps): JSX.Element {
  ...
  const paymentsByBooking = payments.reduce((acc, payment) => {
    ...
    acc[payment.booking_id].total += parseFloat(booking?.offer_amount?.toString() || '0');
    acc[payment.booking_id].paid += payment.status === 'completed' ? parseFloat(payment.amount.toString()) : 0;
    return acc;
  }, {} as Record<string, { booking?: Booking; payments: Payment[]; total: number; paid: number }>);
                ...
                <div className="space-y-2">
                  ...
                  {bookingPs.map((payment) => (
                    ...
                    >
                      ...
                      <span className="font-semibold dark:text-white">{formatCurrency(Number(payment.amount))}</span>
                    </div>
                  ))}
                </div>
              </div>
 ...
 */
function ProfileTab({ clientInfo, user, formatCurrency }: ProfileTabProps): JSX.Element {
  if (!clientInfo) {
    return (
      <div className="bg-white dark:bg-[#2c2e36] rounded-xl border dark:border-gray-700 p-8 text-center">
