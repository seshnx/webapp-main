...
export function useGearOffers(options: { listingId?: string | null; userId?: string | null } = {}) {
  const transactions = useTransactionsByItem(options.listingId || undefined);
...
/**
 * Hook for marketplace mutations (Legacy alias, now combined)
 */
export function useMarketplaceMutations() {
  const itemMutations = useMarketplaceItemMutations();
  const transactionMutations = useTransactionMutations();

  return useMemo(() => ({
    // Item mutations
    createListing: itemMutations.createItem,
    updateListing: itemMutations.updateItem,
    deleteListing: itemMutations.deleteItem,
    updateListingStatus: itemMutations.updateItem, // Alias
    
    // Transaction mutations
    createTransaction: transactionMutations.createTransaction,
    createOffer: transactionMutations.createOffer,
    acceptOffer: transactionMutations.acceptOffer,
    rejectOffer: transactionMutations.rejectOffer,
    completeTransaction: transactionMutations.completeTransaction,
    cancelTransaction: transactionMutations.cancelTransaction,
    addTrackingNumber: transactionMutations.addTrackingNumber,
    updateTransaction: transactionMutations.updateTransaction,
    addPhoto: transactionMutations.addPhoto,
    
    // Legacy compatibility aliases
    createOrder: transactionMutations.createTransaction,
    updateOrderStatus: transactionMutations.updateTransaction,
    respondToOffer: transactionMutations.acceptOffer,
  }), [itemMutations, transactionMutations]);
}
