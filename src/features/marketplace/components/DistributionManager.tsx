...
import ReleaseBuilder from '@/features/business/components/distribution/ReleaseBuilder';
import type { UserData } from '@/types';

/**
 * Release status types
 */
export type ReleaseStatus = 'Draft' | 'Processing' | 'Live' | 'Action Needed';
...
/**
 * Distribution release interface
 */
...
/**
 * Props for DistributionManager component
 */
 ...
 *
 * Manages music distribution releases across platforms like Spotify and Apple Music.
 *
 * @example
 * <DistributionManager
 *   user={currentUser}
 ...
 * />
 */
export default function DistributionManager({ user, userData }: DistributionManagerProps) {
    ...
    const handleDelete = async (releaseId: string, title: string): Promise<void> => {
    ...
    const handleEdit = (release: DistributionRelease): void => {
                            ...
                            <div key={release.id} className="p-5 hover:bg-gray-50 dark:hover:bg-white/5 transition flex flex-col md:flex-row md:items-center justify-between gap-4 group">
                                ...
                                <div className="flex items-center gap-4 pl-4 md:pl-0 border-l md:border-l-0 dark:border-gray-700">
                                    ...
                                    {(release.status === 'Draft' || release.status === 'Action Needed') && (
                                        <button
                                            onClick={() => handleDelete(release.id, release.title || 'Untitled')}
