...
import { MultiSelect, NestedSelect } from '@/components/shared/Inputs';
...
export default function ProfileManager({
    ...
    const { tabOrClerkId, clerkId: urlClerkId, tab: urlTab } = useParams<{ 
        tabOrClerkId?: string; 
        clerkId?: string; 
        tab?: string 
    }>();
    ...
    // Determine active tab and target clerkId from URL
    const knownTabs = ['details', 'settings', 'edit'];
    
    let activeTab = 'details';
    let targetClerkId = userData?.clerkId || user?.id || user?.uid;

    if (urlClerkId && urlTab) {
        targetClerkId = urlClerkId;
        activeTab = urlTab;
    } else if (tabOrClerkId) {
        if (knownTabs.includes(tabOrClerkId)) {
            activeTab = tabOrClerkId;
        } else {
            targetClerkId = tabOrClerkId;
            activeTab = 'details';
        }
    }

    const isOwnProfile = targetClerkId === (userData?.clerkId || user?.id || user?.uid);
    ...
    const handleTabChange = (newTab: string) => {
        if (isOwnProfile) {
            ...
            navigate(`/profile/${targetClerkId}/${newTab}`);
        }
    };
    ...
    return (
        <PageLayout
            title={activeTab === 'details' ? 'Edit Profile' : 'Account Settings'}
            ...
            headerActions={
                <>
                    ...
                    <div className="bg-gray-100 dark:bg-gray-800 p-1 rounded-lg flex gap-1">
                        <button onClick={() => setactiveTab('details')} className={`px-4 py-2 text-xs font-bold rounded-md transition-all ${activeTab === 'details' ? 'bg-white dark:bg-[#2c2e36] text-brand-blue shadow-sm' : 'text-gray-500 hover:text-gray-900 dark:hover:text-gray-300'}`}>Edit Details</button>
                        <button onClick={() => setactiveTab('settings')} className={`px-4 py-2 text-xs font-bold rounded-md transition-all flex items-center gap-1 ${activeTab === 'settings' ? 'bg-white dark:bg-[#2c2e36] text-brand-blue shadow-sm' : 'text-gray-500 hover:text-gray-900 dark:hover:text-gray-300'}`}><Settings size={14}/> Settings</button>
                    </div>
                </>
            }
        >
