...
export function EDUDashboard({ userData, eduRole, className = '' }: EDUDashboardProps) {
  ...
  const classes = useClassesBySchool(schoolId as any, 'active');
  ...
  const enrollments = useEnrollmentsByStudent(studentProfile?._id as any, 'active');
  ...
  if (classes === undefined || (isStudent && enrollments === undefined)) {
    return (
