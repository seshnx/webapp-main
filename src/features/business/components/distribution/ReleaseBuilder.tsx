...
interface StepProps {
  ...
  setData: (data: ReleaseFormData) => void;
  user?: {
    id?: string;
    uid?: string;
    [key: string]: any;
...
const Step1Metadata = ({ data, setData }: StepProps): JSX.Element => (
  <div className="space-y-4 animate-in slide-in-from-right-4">
    ...
    <div className="grid grid-cols-2 gap-4">
      ...
      <div>
        ...
        <input
          ...
          onChange={e => setData({ ...data, primaryArtist: e.target.value })}
    ...
    <div className="grid grid-cols-2 gap-4">
      <div>
        ...
        >
          {RELEASE_TYPES.map((t: { id: string; label: string }) => (
    ...
    <div className="grid grid-cols-2 gap-4">
      ...
      <div>
        ...
        <input
          ...
          onChange={e => setData({ ...data, labelName: e.target.value })}
          placeholder="Optional (Default: Independent)"
        />
      </div>
    </div>
  </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center space-y-6 animate-in slide-in-from-right-4">
      ...
      <div className="relative group w-64 h-64 bg-gray-100 dark:bg-[#1f2128] rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-600 flex items-center justify-center overflow-hidden">
        {preview ? (
          <img src={preview} className="w-full h-full object-cover" alt="Artwork preview" />
        ) : (
...
const Step3Tracks = ({ data, setData, user }: StepProps): JSX.Element => {
    ...
    try {
      ...
      if (res?.url) {
        const finalTracks = updatedTracks.map(t =>
          t.id === newTrack.id ? { ...t, fileUrl: res.url, status: 'ready' as const, duration: 0 } : t
        );
  ...
  const updateTrackMeta = (id: number, field: keyof Track, val: any): void => {
    setData({
      ...data,
        ...
        {data.tracks?.map((track, idx) => (
          <div key={track.id} className="bg-white dark:bg-[#1f2128] p-4 rounded-xl border dark:border-gray-700 flex flex-col gap-3">
            <div className="flex justify-between items-center">
              ...
              <button onClick={() => removeTrack(track.id)} className="text-gray-400 hover:text-red-500">
                <X size={16}/>
              </button>
            </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <label className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-300 p-2 bg-gray-50 dark:bg-gray-800 rounded cursor-pointer">
                  <input
                    ...
                    onChange={e => updateTrackMeta(track.id, 'isExplicit', e.target.checked)}
                  /> Explicit
                </label>
                <label className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-300 p-2 bg-gray-50 dark:bg-gray-800 rounded cursor-pointer">
                  <input
                    ...
                    onChange={e => updateTrackMeta(track.id, 'isInstrumental', e.target.checked)}
                  /> Instrumental
                </label>
              </div>
            )}
...
const Step4Stores = ({ data, setData }: StepProps): JSX.Element => {
  ...
  const toggleAll = (): void => {
    if (data.stores?.length === DISTRIBUTION_STORES.length) {
      setData({ ...data, stores: [] });
    } else {
      setData({ ...data, stores: DISTRIBUTION_STORES.map((s: { id: string }) => s.id) });
    }
  };
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-right-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {DISTRIBUTION_STORES.map((store: { id: string; name: string }) => {
          ...
          return (
            <div
              ...
              className={`p-3 rounded-xl border flex items-center gap-3 cursor-pointer transition-all ${
                isSelected
                  ? 'bg-blue-50 border-brand-blue dark:bg-blue-900/20'
                  : 'bg-white dark:bg-[#1f2128] dark:border-gray-700 hover:border-gray-300'
              }`}
            >
              <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${
                isSelected ? 'bg-brand-blue border-brand-blue' : 'border-gray-400'
              }`}>
                {isSelected && <CheckCircle size={12} className="text-white"/>}
              </div>
              <span className={`text-sm font-bold ${
                isSelected ? 'text-brand-blue dark:text-white' : 'text-gray-600 dark:text-gray-400'
              }`}>
                {store.name}
              </span>
            </div>
...
export default function ReleaseBuilder({
  ...
  onCancel,
  onSuccess
}: ReleaseBuilderProps): JSX.Element {
  const [step, setStep] = useState<number>(1);
  const [submitting, setSubmitting] = useState<boolean>(false);

  // Initial State Template
  const [formData, setFormData] = useState<ReleaseFormData>(initialData || {
    title: '',
    primaryArtist: userData ? `${userData.firstName || ''} ${userData.lastName || ''}`.trim() || 'Artist' : 'Artist',
    type: 'single',
    genre: '',
    releaseDate: '',
    labelName: '',
    artworkUrl: '',
    tracks: [],
    stores: DISTRIBUTION_STORES.map((s: { id: string }) => s.id),
    status: 'Draft'
  });
  ...
  const handleFinalSubmit = async (): Promise<void> => {
    ...
    console.warn('ReleaseBuilder submit needs Neon migration');

    setSubmitting(true);
    const userId = user?.id || user?.uid;
    try {
      // TODO: Migrate to Neon database
      // Would use createDistributionRelease() or updateDistributionRelease() from neonQueries
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white dark:bg-[#2c2e36] rounded-2xl shadow-xl overflow-hidden border dark:border-gray-700 min-h-[600px] flex flex-col">
      {/* Migration Notice */}
      <div className="bg-yellow-50 dark:bg-yellow-900/20 border-b border-yellow-200 dark:border-yellow-800 px-4 py-2 text-xs text-yellow-800 dark:text-yellow-200 text-center">
        This component needs Neon database migration. Some features may not work.
      </div>
      ...
      <div className="p-6 border-t dark:border-gray-700 bg-gray-50 dark:bg-[#23262f] flex justify-between items-center">
        {step > 1 ? (
          <button
            ...
            className="flex items-center gap-2 text-gray-500 font-bold hover:text-gray-800 dark:hover:text-white transition"
          >
