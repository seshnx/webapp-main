...
export default function SocialFeed({
  ...
  const { postId, mode: urlMode } = useParams();
  const [reportTarget, setReportTarget] = useState<Post | null>(null);

  // Determine feed mode from path
  const feedMode = useMemo(() => {
    if (urlMode === 'following') return FEED_MODES.FOLLOWING;
    if (urlMode === 'discover') return FEED_MODES.DISCOVER;
