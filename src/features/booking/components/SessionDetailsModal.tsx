...
import { X, Calendar, Clock, MapPin, DollarSign, CheckCircle, Play, Mic2, Car, FileText, LucideIcon } from 'lucide-react';
import { motion } from 'framer-motion';
import toast from 'react-hot-toast';

/**
 * Tracker step configuration
 */
...
/**
 * Status timestamps for tracking booking progress
 */
...
/**
 * Props for SessionDetailsModal component
 */
 ...
 *
 * Displays detailed information about a booking session with a pizza-tracker style
 * progress indicator showing the session status.
 *
 * @example
 * <SessionDetailsModal
 *   booking={bookingData}
...
export default function SessionDetailsModal({
    ...
    const isProvider = (user.uid || user.id) === booking.targetId; // The person who was booked (Talent/Studio)
    const isClient = (user.uid || user.id) === booking.senderId; // The person who booked

    // Helper to get current step index (0-4)
    const getCurrentStepIndex = (currentStatus: string): number => {
    ...
    const updateStatus = async (newStatus: string): Promise<void> => {
        setLoading(true);
        try {
            // TODO: Implement Neon status update
                                ...
                                return (
                                    <div key={step.id} className="flex flex-col items-center gap-2">
                                        <div
                                            ...
                                            `}
                                        >
                                            <StepIcon size={isCurrent ? 18 : 16} fill={isCompleted ? "currentColor" : "none"} />
                                        </div>
