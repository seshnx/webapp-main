...
export default function EduEvaluations({ schoolId, logAction, graderName }: EduEvaluationsProps) {
    ...
    // Convex Queries
    const studentsData = useQuery(api.edu.getStudentsBySchool, 
        schoolId ? { schoolId: schoolId as Id<"schools"> } : "skip"
    );

    const historyData = useQuery(api.edu.getEvaluationsByStudent,
        (schoolId && selectedStudent) 
            ? { schoolId: schoolId as Id<"schools">, studentId: selectedStudent.id as Id<"users"> } 
            : "skip"
    );

    // Convex Mutations
    const submitEvaluation = useMutation(api.edu.createEvaluation);
    ...
    const studentHistory: Evaluation[] = useMemo(() => {
    ...
    const handleSubmitGrade = async () => {
        ...
        try {
            await submitEvaluation({
                ...
                studentId: selectedStudent.id as Id<"users">,
                technical: gradeForm.technical,
                softSkills: gradeForm.softSkills,
            ...
            setShowForm(false);
            setGradeForm({ technical: 5, softSkills: 5, notes: '' });
    ...
    if (studentsData === undefined) return <div className="p-10 text-center text-gray-500">Loading Roster...</div>;
