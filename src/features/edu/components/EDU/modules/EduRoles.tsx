 ...
 * EduRoles props
 */
export interface EduRolesProps {
    schoolId?: string;
    logAction?: (action: string, details: string) => Promise<void> | void;
}
export default function EduRoles({ schoolId, logAction }: EduRolesProps) {
    // Modal / Form State
    const [showCreateModal, setShowCreateModal] = useState<boolean>(false);
    const [roleForm, setRoleForm] = useState<RoleForm>({ name: '', color: '#3b82f6', permissions: [] });
    ...
    // Convex Mutations
    const addRole = useMutation(api.edu.createSchoolRole);
    const updatePerms = useMutation(api.edu.updateSchoolRolePermissions);
    const removeRole = useMutation(api.edu.deleteSchoolRole);

    const roles: SchoolRole[] = useMemo(() => {
        if (!rolesData) return [];
        return rolesData.map(r => ({ id: r._id, ...r }));
    }, [rolesData]);

    // --- ACTIONS ---

    const handleCreateRole = async () => {
        if (!roleForm.name || !schoolId) return;
        setSaving(true);
        try {
    ...
    const handleDeleteRole = async (roleId: string, roleName: string) => {
        ...
        try {
            await removeRole({ roleId: roleId as Id<"schoolRoles"> });
    ...
    const handleUpdatePermissions = async (roleId: string, newPermissions: string[]) => {
        try {
            await updatePerms({
                roleId: roleId as Id<"schoolRoles">,
    ...
    const toggleExistingRolePermission = (role: SchoolRole, permId: string) => {
    ...
    if (rolesData === undefined) return <div className="p-10 text-center text-gray-500">Loading Roles...</div>;
                ...
                {roles.map(role => (
                    <div key={role.id} className="bg-white dark:bg-[#2c2e36] border dark:border-gray-700 rounded-xl shadow-sm flex flex-col overflow-hidden">
                        ...
                        <div className="p-4 border-b dark:border-gray-700 flex justify-between items-start bg-gray-50 dark:bg-gray-800/50">
                            ...
                            <button
                                onClick={() => handleDeleteRole(role.id, role.name || 'this role')}
                        ...
                        <div className="p-2 flex-1 overflow-y-auto max-h-[300px] custom-scrollbar">
                            <div className="space-y-0.5">
                                {SCHOOL_PERMISSIONS.map((perm: any) => {
