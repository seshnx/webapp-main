 ...
 * EduResources props
 */
export interface EduResourcesProps {
    schoolId?: string;
    logAction?: (action: string, details: string) => Promise<void> | void;
}

export default function EduResources({ schoolId, logAction }: EduResourcesProps) {
    // Form State
    ...
    // Convex Queries
    const rulesData = useQuery(api.edu.getResourceRulesBySchool, 
        schoolId ? { schoolId: schoolId as Id<"schools"> } : "skip"
    );

    // Convex Mutations
    const addRule = useMutation(api.edu.createResourceRule);
    const removeRule = useMutation(api.edu.deleteResourceRule);

    const rules: ResourceRule[] = useMemo(() => {
        if (!rulesData) return [];
        return rulesData.map(r => ({ id: r._id, ...r }));
    }, [rulesData]);

    // --- ACTIONS ---

    const handleAddRule = async () => {
        if (!ruleForm.resource || !schoolId) {
    ...
    const handleDeleteRule = async (id: string, resourceName: string) => {
        if(!confirm("Delete this rule?") || !schoolId) return;
        try {
            await removeRule({ ruleId: id as Id<"resourceRules"> });
    ...
    if (rulesData === undefined) return <div className="p-10 text-center text-gray-500">Loading Rules...</div>;
                    ...
                    <div>
                        ...
                        <div className="flex gap-2">
                            <input
                                ...
                                onChange={e => setRuleForm({...ruleForm, limit: parseInt(e.target.value) || 0})}
                            />
                            <select
                ...
                {rules.length === 0 ? (
                    ...
                    rules.map(r => (
                        <div key={r.id} className="bg-white dark:bg-[#2c2e36] p-4 rounded-xl border dark:border-gray-700 flex justify-between items-center shadow-sm hover:shadow-md transition">
                            ...
                            <button
                                onClick={() => handleDeleteRule(r.id, r.resource || 'this resource')}
