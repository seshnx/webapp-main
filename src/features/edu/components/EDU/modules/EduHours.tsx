 ...
 * EduHours props
 */
export interface EduHoursProps {
    schoolId?: string;
}
export default function EduHours({ schoolId }: EduHoursProps) {
    const [filterStatus, setFilterStatus] = useState<string>('all'); // 'all', 'pending_approval', 'approved'

    // Convex Queries
    const logsData = useQuery(api.edu.getInternshipLogsBySchool,
        schoolId ? { schoolId: schoolId as Id<"schools">, limit: 100 } : "skip"
    );

    // Convex Mutations
    const updateStatus = useMutation(api.edu.updateInternshipLogStatus);

    const logs: InternshipLog[] = useMemo(() => {
        if (!logsData) return [];
        return logsData.map(log => ({
            id: log._id,
            ...log,
            // duration is already calculated in convex if we want, or here
            duration: log.duration || calculateDurationMinutes(log.checkIn, log.checkOut)
        }));
    }, [logsData]);

    // --- ACTIONS ---

    const handleLogAction = async (logId: string, status: string) => {
        try {
            await updateStatus({
                logId: logId as Id<"internshipLogs">,
                status
            });
        } catch (error) {
    ...
    if (logsData === undefined) return <div className="p-10 text-center text-gray-500">Loading Internship Hours...</div>;
