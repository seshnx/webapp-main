...
/**
 * EduLearningPaths props
 */
export interface EduLearningPathsProps {
    schoolId?: string;
    logAction?: (action: string, details: any) => Promise<void> | void;
}
export default function EduLearningPaths({ schoolId, logAction }: EduLearningPathsProps) {
    ...
    // Convex Queries
    const pathsData = useQuery(api.edu.getLearningPathsBySchool,
        schoolId ? { schoolId: schoolId as Id<"schools"> } : "skip"
    );

    const coursesData = useQuery(api.edu.getCoursesBySchool,
        schoolId ? { schoolId: schoolId as Id<"schools"> } : "skip"
    );

    // Convex Mutations
    const createPath = useMutation(api.edu.createLearningPath);
    const updatePath = useMutation(api.edu.updateLearningPath);
    const removePath = useMutation(api.edu.deleteLearningPath);
    ...
    const paths: LearningPath[] = useMemo(() => {
        ...
        return pathsData.map(p => ({
            ...
            mentorshipEnabled: p.mentorshipEnabled || false
        }));
    }, [pathsData]);

    const availableCourses: AvailableCourse[] = useMemo(() => {
    ...
    const handleSavePath = async () => {
        if (!formData.title.trim() || !schoolId) {
        ...
        try {
            if (editingPath) {
                await updatePath({
                    pathId: editingPath.id as Id<"learningPaths">,
                    title: formData.title,
                    description: formData.description,
                    courses: formData.courses,
                    badges: formData.badges,
                    skills: formData.skills,
                    mentorshipEnabled: formData.mentorshipEnabled
                });
                ...
                if (logAction) {
                    logAction('create_learning_path', { pathId, title: formData.title });
                }
            }
    ...
    const handleDeletePath = async (pathId: string) => {
        if (!confirm('Are you sure you want to delete this learning path?') || !schoolId) {
            return;
        }

        try {
            await removePath({ pathId: pathId as Id<"learningPaths"> });
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-4">
                    ...
                    <div className="flex items-center gap-2">
                        <input
                            ...
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormData({ ...formData, mentorshipEnabled: e.target.checked })}
                            className="rounded"
                        />
