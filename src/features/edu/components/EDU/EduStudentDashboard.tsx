...
/**
 * EduStudentDashboard props
 */
export interface EduStudentDashboardProps {
    user?: any;
    userData?: any;
}
export default function EduStudentDashboard({ user: propUser, userData: propUserData }: EduStudentDashboardProps) {
    ...
    const interests: string[] = userData?.internshipCities || [];
    const schoolId = userData?.schoolId;
    const userId = user?.id || user?.uid;

    // Convex Queries
    const studiosData = useQuery(api.studios.getActiveStudios, {});
    const classesData = useQuery(api.edu.getClassesBySchool, 
        schoolId ? { schoolId: schoolId as Id<"schools"> } : "skip"
    );
    const enrollmentsData = useQuery(api.edu.getEnrollmentsByStudent, 
        userId ? { studentId: userId as Id<"users"> } : "skip"
    );
    const badgesData = useQuery(api.edu.getBadgesByStudent, 
        userId ? { studentId: userId as Id<"users"> } : "skip"
    );

    // Convex Mutations
    const updateProfile = useMutation(api.users.updateAllProfiles);
    const enrollInClass = useMutation(api.edu.enrollStudent);

    const studios: Studio[] = useMemo(() => {
        if (!studiosData) return [];
        let results = studiosData.map(s => ({
            id: s._id,
            ...s,
            zipCode: s.zip
        }));
        ...
        if (interests.length > 0) {
            results = results.filter((studio: Studio) =>
        ...
        if (trimmed && interests.length < 3) {
            try {
                ...
                await updateProfile({
                    ...
                    mainProfileUpdates: { internshipCities: updatedInterests }
                });
                setNewInterest('');
    ...
    const handleEnroll = async (courseId: string) => {
        ...
        try {
            await enrollInClass({
                ...
                classId: courseId as Id<"classes">
            });
        } catch (error) {
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-8 pb-20">
            ...
            {userData?.programComplete && (
                ...
                {studiosData === undefined ? <p className="text-gray-500 flex items-center gap-2"><Clock className="animate-spin" size={16}/> Searching...</p> : (
                        studios.length > 0 ? (
