...
export function useReadReceipts(
  ...
  const markAsReadMutation = useMutation(api.messenger.markMessageAsRead);
  ...
  const markAsRead = useCallback(async (messageIds: string | string[]): Promise<void> => {
    if (!chatId || !currentUserId || !messageIds || !isConvexAvailable() || !markAsReadMutation) return;

    try {
      const messageIdArray = Array.isArray(messageIds) ? messageIds : [messageIds];
      
      // Use the latest message ID to mark as read
      // messenger.markMessageAsRead will set unreadCount to 0 for this user
      const latestMessageId = messageIdArray[messageIdArray.length - 1];

      await markAsReadMutation({
        messageId: latestMessageId as any,
        userId: currentUserId,
      });
    } catch (error) {
      console.error('Mark as read error:', error);
    }
  }, [chatId, currentUserId, markAsReadMutation]);
