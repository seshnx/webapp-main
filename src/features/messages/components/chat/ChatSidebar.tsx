...
import ProfileSelectionModal from '@/components/shared/ProfileSelectionModal';
...
export default function ChatSidebar({ user, userData, subProfiles = {}, conversations = [], activeChat, onSelectChat }: ChatSidebarProps) {
    ...
    // Convex mutations
    const startDirectMessageMutation = useMutation(api.messenger.startDirectMessage);
    ...
    const handleProfileConfirmed = async (role: string) => {
        setSelectedChatProfile(role);
        if (pendingChatTarget) {
            try {
                const userId = user?.id || user?.uid;
                
                // Call Convex mutation to start/get direct message conversation
                // This ensures the conversation exists in the backend and returns the correct chatId
                const { chatId } = await startDirectMessageMutation({
                    initiatorId: userId,
                    recipientId: pendingChatTarget.id,
                });
